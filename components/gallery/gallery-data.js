export const projectData = [
  { 
    homepage: true,
    id: 0,
    project: "FROM", 
    company: "MGM+",
    src: "/images/cover-art/from.jpg", 
    hero: "bg-[url('/images/hero-from.jpg')]",
    title: "", 
    color: "bg-[#192427]", 
    subtitle: "", 
    cta: "Watch on MGM+",
    ctabrand: "bg-[#c7a338] hover:bg-[#c7a338]", 
    ctabrandhover: "",
    ctaicon: "PlayCircleIcon", 
    ctalink: "https://www.epix.com/series/from", 
    year: "2022", 
    desc: "'FROM' unravels the mystery of a nightmarish town that traps all those who enter. As the unwilling residents fight to keep a sense of normalcy and search for a way out, they must also survive the threats of the surrounding forest – including the terrifying creatures that come out when the sun goes down.", 
    songsdata: [
      {
        "title": "From Main Theme",
        "url": "/music/christilton_from_01_from-main-theme.mp3" 
      },
      {
        "title": "Driving in Circles",
        "url": "/music/christilton_from_02_driving-in-circles.mp3" 
      },
      {
        "title": "Stone in the Cellar",
        "url": "/music/christilton_from_03_stone-in-the-cellar.mp3" 
      },
      {
        "title": "Pray with Me",
        "url": "/music/christilton_from_04_pray-with-me.mp3" 
      },
      {
        "title": "Messages in the Bottles",
        "url": "/music/christilton_from_05_messages-in-the-bottles.mp3"
      }
    ],
  },
  { 
    id: 1, 
    ctalink: "/projects", 
    project: "Moxie", 
    company: "Embodied, Inc", 
    src: "/images/cover-art/moxie.jpg", 
    hero: "bg-[url('/images/hero-moxie.jpg')]",
    title: "Moxie (Robotic Companion) 2021", 
    color: "bg-[#00a1b7]", 
    subtitle: "", 
    cta: "Learn More about Moxie",
    ctabrand: "", 
    ctabrandhover: "",
    ctaicon: "none", 
    year: "2021", 
    desc: "Developed by a veteran team of technologists, neuroscientists, child development specialists, and creative storytellers, Moxie is a social robot designed with the latest technology that allows it to engage with children in a revolutionary way. ", 
    songsdata: [
      {
        "title": "The Adventures of Moxie (from the robot companion 'Moxie')",
        "url": "/music/christilton_moxie_01_the-adventures-of-moxie.mp3" 
      },
      {
        "title": "Oh, the Places You'll Go! (from the robot companion 'Moxie')",
        "url": "/music/christilton_moxie_02_oh-the-places-youll-go.mp3" 
      },
      {
        "title": "The Library (from the robot companion 'Moxie')",
        "url": "/music/christilton_moxie_03_the-library.mp3" 
      },
      {
        "title": "Moxie, P.I. (from the robot companion 'Moxie')",
        "url": "/music/christilton_moxie_04_moxie-p-i.mp3" 
      },
      {
        "title": "Global Robotics Laboratories (from the robot companion 'Moxie')",
        "url": "/music/christilton_moxie_05_global-robotics-laboratories.mp3"
      }
    ]
  },
  { 
    id: 2, 
    ctalink: "/projects", 
    project: "CSC", 
    company: "Lucid Sight, Inc", 
    src: "/images/cover-art/csc.jpg", 
    hero: "bg-[url('/images/hero-csc.jpg')]",
    title: "CSC (Video Game) 2019", 
    color: "bg-[#849CA8]", 
    subtitle: "", 
    cta: "Listen on Spotify", 
    ctabrand: "bg-[#1DB954] hover:bg-[#1DB954]",
    ctabrandhover: "",
    ctaicon: "icon-spotify.png", 
    year: "2019", 
    desc: "CSC is an open-universe, sandbox-style space MMO with a player-owned economy. Command starships and earn digital items to buy, sell, or trade without limits both inside and outside of the game universe. Experience a free-market Play-to-Own Economy that provides true ownership & freedom.", 
    songsdata: [
      {
        "title": "Main Theme (from the video game 'CSC')",
        "url": "/music/christilton_csc_01_csc-main-theme.mp3" 
      },
      {
        "title": "Nova Waves (from the video game 'CSC')",
        "url": "/music/christilton_csc_02_nova-waves.mp3" 
      },
      {
        "title": "The Galactic Grind (from the video game 'CSC')",
        "url": "/music/christilton_csc_03_the-galactic-grind.mp3" 
      },
      {
        "title": "Verse Traveler (from the video game 'CSC')",
        "url": "/music/christilton_csc_04_verse-traveler.mp3" 
      },
      {
        "title": "Epilogue (from the video game 'CSC')",
        "url": "/music/christilton_csc_05_epilogue.mp3"
      }
    ]
  },
  { 
    homepage: true,
    id: 3, 
    ctalink: "/projects", 
    project: "Zoo", 
    company: "", 
    src: "/images/cover-art/zoo.jpg", 
    hero: "bg-[url('/images/hero-zoo.jpg')]",
    title: "", 
    color: "bg-[#6a9881]", 
    subtitle: "", 
    cta: "Watch on Paramount+",
    ctabrand: "bg-[#0164FF] hover:bg-[#0164FF]", 
    ctabrandhover: "",
    ctaicon: "",
    year: "2018", 
    desc: `Zoo, based on the #1 bestselling novel by James Patterson, is a global thriller about a wave of violent animal attacks against humans sweeping the planet. In season three, the year is 2027, and the world has reached a tipping point: as the human population continues to dwindle due to the sterility problem, an equally devastating threat rises in the form of "the hybrids," an army of unstoppable lab-made creatures, hell-bent on destroying mankind. Although the team has become estranged over the past decade, Jackson, Jamie, Abraham, Dariela and Logan are forced to overcome their differences and reunite when they learn that Mitch is alive. As the planet’s future hangs in the balance, the team must race to stop the hybrids and their creator - a shadowy figure with a startling connection to the team's past.`,
    songsdata: [
      {
        "title": "Welcome to Pangea (from the TV series 'Zoo')",
        "url": "/music/christilton_from_01_from-main-theme.mp3" 
      },
      {
        "title": "Driving in Circles (from the TV series 'From')",
        "url": "/music/christilton_from_02_driving-in-circles.mp3" 
      },
      {
        "title": "Stone in the Cellar (from the TV series 'From')",
        "url": "/music/christilton_from_03_stone-in-the-cellar.mp3" 
      },
      {
        "title": "Pray with Me (from the TV series 'From')",
        "url": "/music/christilton_from_04_pray-with-me.mp3" 
      },
      {
        "title": "Messages in the Bottles (from the TV series 'From')",
        "url": "/music/christilton_from_05_messages-in-the-bottles.mp3"
      }
    ],
  },  
  { 
    id: 4, 
    ctalink: "/projects", 
    project: "Divide", 
    company: "Exploding Tuba Studios", 
    src: "/images/cover-art/divide.jpg", 
    hero: "bg-[url('/images/hero-divide.jpg')]",
    title: "Divide (Video Game) 2017", 
    color: "bg-[#849CA8]", 
    subtitle: "", 
    cta: "Listen on Spotify", 
    ctabrand: "bg-[#1DB954] hover:bg-[#1DB954]",
    ctabrandhover: "",
    ctaicon: "icon-spotify.png", 
    year: "2017", 
    desc: "Divide is a character-driven, science fiction action/adventure game with a modern take on isometric adventures of the past. Search for clues that reveal new insight into the story, interact with fully realized characters, and combat enemies by hacking out of sight, or launching a direct assault.", 
    songsdata: [
      {
        "title": "Divide Main Theme (from the video game 'Divide')",
        "url": "/music/christilton_divide_01_divide-main-theme.mp3" 
      },
      {
        "title": "Searching for Hashes (from the video game 'Divide')",
        "url": "/music/christilton_divide_02_searching-for-hashes.mp3" 
      },
      {
        "title": "The Last Stop (from the video game 'Divide')",
        "url": "/music/christilton_divide_03_the-last-stop.mp3" 
      },
      {
        "title": "Unit Prime (from the video game 'Divide')",
        "url": "/music/christilton_divide_04_unit-prime.mp3" 
      },
      {
        "title": "Divided (feat. Tori Letzler) (from the video game 'Divide')",
        "url": "/music/christilton_divide_05_divided.mp3"
      }
    ]
  },
  
  { 
    homepage: true,
    id: 5, 
    ctalink: "/projects", 
    project: "Assassin's Creed Unity", 
    company: "", 
    src: "/images/cover-art/assassins-creed-unity.jpg", 
    hero: "bg-[url('/images/hero-ac.jpg')]",
    title: "", 
    color: "bg-[#849CA8]", 
    subtitle: "", 
    cta: "Listen on Spotify",
    ctabrand: "bg-[#1DB954] hover:bg-[#1DB954]", 
    ctabrandhover: "",
    ctaicon: "", 
    year: "2021-2022", 
    desc: "Assassin's Creed® Unity is an action/adventure game set in the city of Paris during one of its darkest hours, the French Revolution. Take ownership of the story by customising Arno's equipement to make the experience unique to you, both visually and mechanically. In addition to an epic single-player experience, Assassin’s Creed Unity delivers the excitement of playing with up to three friends through online cooperative gameplay in specific missions. Throughout the game, take part in one of the most pivotal moments of French history in a compelling storyline and a breath-taking playground that brought you the city of lights of today.", 
    songsdata: [
      {
        "title": "Unity (from the video game 'Assassin's Creed Unity') [Excerpt]",
        "url": "/music/christilton_acunity_01_unity_excerpt.mp3" 
      },
      {
        "title": "On Father's Watch (from the video game 'Assassin's Creed Unity') [Excerpt]",
        "url": "/music/christilton_acunity_02_on-fathers-watch_excerpt.mp3" 
      },
      {
        "title": "A Leap of Faith (from the video game 'Assassin's Creed Unity') [Excerpt]",
        "url": "/music/christilton_acunity_03_a-leap-of-faith_excerpt.mp3" 
      },
      {
        "title": "Versailles for Sore Eyes (from the video game 'Assassin's Creed Unity') [Excerpt]",
        "url": "/music/christilton_acunity_04_versailles-for-sore-eyes_excerpt.mp3" 
      },
      {
        "title": "A Clash of Assassins (from the video game 'Assassin's Creed Unity') [Excerpt]",
        "url": "/music/christilton_acunity_05_a-clash-of-assassins_excerpt.mp3"
      }
    ],
  },
  { 
    id: 6, 
    ctalink: "/projects", 
    project: "SimCity: Cities of Tomorrow", 
    company: "Electronic Arts", 
    src: "/images/cover-art/sim-city-cities-of-tomorrow.jpg", 
    hero: "bg-[url('/images/hero-simcity-cot.jpg')]",
    title: "SimCity: Cities of Tomorrow (Video Game) 2013", 
    color: "bg-[#849CA8]", 
    subtitle: "", 
    cta: "Listen on Spotify", 
    ctabrand: "bg-[#1DB954] hover:bg-[#1DB954]",
    ctabrandhover: "",
    ctaicon: "icon-spotify.png", 
    year: "2013", 
    desc: "Will you create a utopian society underpinned by clean technology, or allow a giant corporation to plunder and pollute in the name of feeding your Sims’ insatiable consumerism? In addition to expanding outward, cities will have the ability to build into the sky with enormous multi-zone MegaTowers. Education and research will help you discover new technologies that make your cities less polluted, less reliant on natural resources, managed day-to-day by service drones, and as a byproduct– at risk for resource-draining giant robot attacks on your city. As the population increases, your Sims will live, work, and play closer together. Whether they do so in harmony and prosperity or as members of an exploited workforce is up to you.", 
    songsdata: [
      {
        "title": "Cities of Tomorrow (from the video game 'SimCity: Cities of Tomorrow') [Excerpt]",
        "url": "/music/christilton_simcitycot_01_cities-of-tomorrow_excerpt.mp3" 
      },
      {
        "title": "SimCity: November 2019 (from the video game 'SimCity: Cities of Tomorrow') [Excerpt]",
        "url": "/music/christilton_simcitycot_02_simcity-november-2019_excerpt.mp3" 
      },
      {
        "title": "Population: 1 - Future Mix (from the video game 'SimCity: Cities of Tomorrow') [Excerpt]",
        "url": "/music/christilton_simcitycot_03_population-1-future_excerpt.mp3" 
      },
      {
        "title": "Clear Skyscrapers - Future Mix (from the video game 'SimCity: Cities of Tomorrow') [Excerpt]",
        "url": "/music/christilton_simcitycot_08_clear-skyscrapers-future_excerpt.mp3" 
      },
      {
        "title": "Metropolis Made Easy - Future Mix (from the video game 'SimCity: Cities of Tomorrow') [Excerpt]",
        "url": "/music/christilton_simcitycot_10_metropolis-made-easy-future_excerpt.mp3"
      }
    ]
  },
  { 
    homepage: true,
    id: 7, 
    ctalink: "/projects", 
    project: "Sim City", 
    company: "", 
    src: "/images/cover-art/sim-city.jpg", 
    hero: "bg-[url('/images/hero-simcity.jpg')]",
    title: "", 
    color: "bg-[#59c4dd]", 
    subtitle: "", 
    cta: "Listen on Spotify",
    ctabrand: "bg-[#1DB954] hover:bg-[#1DB954]", 
    ctabrandhover: "",
    ctaicon: "", 
    year: "2019", 
    desc: "Do you have what it takes to build a metropolis from scratch? Find out in SimCity, a game that tasks you with planning the layout and infrastructure of a fully simulated city. Create a single city or up to 16 cities at once, then track the education, wealth, happiness, and more of your region and the Sims that live in it. Anyone can build a city - but it takes a good mayor to make one thrive.", 
    songsdata: [
      {
        "title": "Sim City Theme (from the video game 'Sim City')",
        "url": "/music/christilton_from_01_from-main-theme.mp3" 
      },
      {
        "title": "Driving in Circles (from the TV series 'From')",
        "url": "/music/christilton_from_02_driving-in-circles.mp3" 
      },
      {
        "title": "Stone in the Cellar (from the TV series 'From')",
        "url": "/music/christilton_from_03_stone-in-the-cellar.mp3" 
      },
      {
        "title": "Pray with Me (from the TV series 'From')",
        "url": "/music/christilton_from_04_pray-with-me.mp3" 
      },
      {
        "title": "Messages in the Bottles (from the TV series 'From')",
        "url": "/music/christilton_from_05_messages-in-the-bottles.mp3"
      }
    ],
  },
  
  { 
    homepage: true,
    id: 8, 
    ctalink: "/projects", 
    project: "Fringe", 
    company: "", 
    src: "/images/cover-art/fringe-season-4.jpg", 
    hero: "bg-[url('/images/hero-fringe.jpg')]",
    title: "", 
    color: "bg-[#2160ab]", 
    subtitle: "", 
    cta: "Listen on Spotify",
    ctabrand: "bg-[#1DB954] hover:bg-[#1DB954]", 
    ctabrandhover: "",
    ctaicon: "", 
    year: "2020", 
    desc: "The critically acclaimed FRINGE explores the ever-blurring line between science fiction and reality, where hybrid monsters tear through sewers, thieves walk through walls and portals open to parallel universes. Unable to police a world in which science has advanced beyond our wildest dreams -- and nightmares -- FBI Special Agent Olivia Dunham seeks help from eccentric 'fringe' scientist Dr. Walter Bishop and his jack-of-all trades son, Peter. Under the direction of Special Agent Phillip Broyles and assisted by Agents Astrid Farnsworth, Charlie Francis and Lincoln Lee, the Fringe Team investigates unusual incidents that defy human logic and unimaginable events that threaten our very existence on a universal scale.", 
    songsdata: [
      {
        "title": "A New Day in the Old Town (from the TV series 'Fringe')",
        "url": "/music/christilton_from_01_from-main-theme.mp3" 
      },
      {
        "title": "Driving in Circles (from the TV series 'From')",
        "url": "/music/christilton_from_02_driving-in-circles.mp3" 
      },
      {
        "title": "Stone in the Cellar (from the TV series 'From')",
        "url": "/music/christilton_from_03_stone-in-the-cellar.mp3" 
      },
      {
        "title": "Pray with Me (from the TV series 'From')",
        "url": "/music/christilton_from_04_pray-with-me.mp3" 
      },
      {
        "title": "Messages in the Bottles (from the TV series 'From')",
        "url": "/music/christilton_from_05_messages-in-the-bottles.mp3"
      }
    ],
  },
  { 
    id: 9, 
    ctalink: "/projects", 
    project: "Alcatraz", 
    company: "Warner Bros/Bad Robot", 
    src: "/images/cover-art/alcatraz.jpg", 
    hero: "bg-[url('/images/hero-alcatraz.jpg')]",
    title: "Alcatraz (TV Series) 2012", 
    color: "bg-[#849CA8]", 
    subtitle: "Co-Composed with Andrea Datzman, Michael Giacchino", 
    cta: "", 
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "", 
    year: "2012", 
    desc: "Alcatraz is a science fiction crime drama TV series where 63 inmates from the infamous Alcatraz prison reappear in the present day after mysteriously disappearing in 1963. A special FBI task force investigates their return and uncovers dark secrets and a conspiracy surrounding their reappearance. The show blends mystery, action, and time-travel elements in a compelling storyline.", 
    songsdata: [
      {
        "title": "Where There's Smoke There's Salmon (from the TV series 'Alcatraz')",
        "url": "/music/christilton_alcatraz_01_where-theres-smoke-theres-salmon.mp3" 
      },
      {
        "title": "The Little Rock of Horrors (from the TV series 'Alcatraz')",
        "url": "/music/christilton_alcatraz_02_the-little-rock-of-horrors.mp3" 
      },
      {
        "title": "Emerson, Lucy, and Beauregard (from the TV series 'Alcatraz')",
        "url": "/music/christilton_alcatraz_03_emerson-lucy-and-beauregard.mp3" 
      },
      {
        "title": "Not a Still Man (from the TV series 'Alcatraz')",
        "url": "/music/christilton_alcatraz_04_not-a-still-man.mp3" 
      },
      {
        "title": "Killer Kit (from the TV series 'Alcatraz')",
        "url": "/music/christilton_alcatraz_05_killer-kit.mp3"
      }
    ]
  },
  { 
    id: 10, 
    ctalink: "/projects", 
    project: "Cover", 
    company: "Directed by Josh Negrin", 
    src: "/images/cover-art/cover.jpg", 
    hero: "bg-[url('/images/hero-cover.jpg')]",
    title: "Cover (Short Film) 2011", 
    color: "bg-[#849CA8]", 
    subtitle: "", 
    cta: "Download Soundtrack", 
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "icon-download.png", 
    year: "2011", 
    desc: "Jake and Sara's day together takes a turn for the worse, when they're suddenly being chased and shot at by government spooks.", 
    songsdata: [
      {
        "title": "Opening Titles (from the short film 'Cover)",
        "url": "/music/christilton_cover_01_opening-titles.mp3" 
      },
      {
        "title": "CIA (from the short film 'Cover)",
        "url": "/music/christilton_cover_02_cia.mp3" 
      },
      {
        "title": "Art of the Sneak (from the short film 'Cover)",
        "url": "/music/christilton_cover_03_art-of-the-sneak.mp3" 
      },
      {
        "title": "Gunfight (from the short film 'Cover)",
        "url": "/music/christilton_cover_04_gunfight.mp3" 
      },
      {
        "title": "Finale (from the short film 'Cover)",
        "url": "/music/christilton_cover_05_finale.mp3"
      }
    ]
  },
  { 
    id: 11, 
    ctalink: "/projects", 
    project: "Undercovers", 
    company: "Warner Bros/Bad Robot", 
    src: "/images/cover-art/undercovers.jpg", 
    hero: "bg-[url('/images/hero-undercovers.jpg')]",
    title: "Undercovers (TV Series) 2010", 
    color: "bg-[#849CA8]", 
    subtitle: "Co-Composed with Andrea Datzman, Michael Giacchino", 
    cta: "", 
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "", 
    year: "2010", 
    desc: "Follow the lives of Steven and Samantha Bloom, a married couple and both retired CIA agents. After a former colleague goes missing, they are reluctantly pulled back into the world of espionage to locate and rescue him. The series follows their thrilling missions around the globe, balancing the challenges of married life with the high-stakes nature of their spy work, all while uncovering secrets from their past.", 
    songsdata: [
      {
        "title": "Brothers in Harms (from the TV series 'UnderCovers')",
        "url": "/music/christilton_undercovers_01_brothers-in-harms.mp3" 
      },
      {
        "title": "Pyongyang'ked (from the TV series 'UnderCovers')",
        "url": "/music/christilton_undercovers_02_pyongyangked.mp3" 
      },
      {
        "title": "Villa de Becker (from the TV series 'UnderCovers')",
        "url": "/music/christilton_undercovers_03_villa-de-becker.mp3" 
      },
      {
        "title": "Clive Arrives (from the TV series 'UnderCovers')",
        "url": "/music/christilton_undercovers_04_clive-arrives.mp3" 
      },
      {
        "title": "Act Natural (from the TV series 'UnderCovers')",
        "url": "/music/christilton_undercovers_05_act-natural.mp3"
      }
    ]
  },
  { 
    id: 12, 
    ctalink: "/projects", 
    project: "Night at the Museum: Battle of the Smithsonian", 
    company: "Majesco/Pipeworks", 
    src: "/images/cover-art/night-at-the-museum.jpg", 
    hero: "bg-[url('/images/hero-night-at-the-museum.jpg')]",
    title: "Night at the Museum: Battle of the Smithsonian (Video Game) 2009", 
    color: "bg-[#849CA8]", 
    subtitle: "", 
    cta: "Download Soundtrack", 
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "icon-download.png", 
    year: "2009", 
    desc: "Assume the role of the night security guard, Larry Daley, who must navigate through the vast and lively halls of the Smithsonian Institution to rescue his museum friends, who have been transported into the museum exhibits by the mystical Tablet of Ahkmenrah. As players explore various exhibits, they encounter historical figures and famous characters that come to life, leading to hilarious and exciting adventures. The game combines puzzle-solving, exploration, and action elements, providing an enjoyable journey through history with a touch of whimsy and humor inspired by the movie.", 
    songsdata: [
      {
        "title": "Main Theme (from the video game 'Night at the Museum: Battle of the Smithsonian')",
        "url": "/music/christilton_natm2_01_main-theme.mp3" 
      },
      {
        "title": "Larry the Inventor (from the video game 'Night at the Museum: Battle of the Smithsonian')",
        "url": "/music/christilton_natm2_02_larry-the-inventor.mp3" 
      },
      {
        "title": "The Federal Archive (from the video game 'Night at the Museum: Battle of the Smithsonian')",
        "url": "/music/christilton_natm2_03_the-federal-archives.mp3" 
      },
      {
        "title": "Ingot Me a Painting (from the video game 'Night at the Museum: Battle of the Smithsonian')",
        "url": "/music/christilton_natm2_04_ingot-me-a-painting.mp3" 
      },
      {
        "title": "Battle of the Smithsonian (from the video game 'Night at the Museum: Battle of the Smithsonian')",
        "url": "/music/christilton_natm2_05_battle-of-the-smithsonian.mp3"
      }
    ]
  },
  { 
    id: 13, 
    ctalink: "/projects", 
    project: "Fracture", 
    company: "LucasArts/Day 1 Studios", 
    src: "/images/cover-art/fracture.jpg", 
    hero: "bg-[url('/images/hero-fracture.jpg')]",
    title: "Fracture (Video Game) 2008", 
    color: "bg-[#849CA8]", 
    subtitle: "Co-composed with Chad Seiter, Michael Giacchino", 
    cta: "Download Soundtrack", 
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "icon-download.png", 
    year: "2008", 
    desc: "Fracture is a third-person shooter video game developed by Day 1 Studios and published by LucasArts. Set in a futuristic world, the game features innovative terrain deformation mechanics that allow players to reshape the landscape and create strategic advantages during intense battles between two warring factions.", 
    songsdata: [
      {
        "title": "The Pacificans (from the video game 'Fracture')",
        "url": "/music/christilton_fracture_01_the-pacificans.mp3" 
      },
      {
        "title": "Mariko (from the video game 'Fracture')",
        "url": "/music/christilton_fracture_02_mariko.mp3" 
      },
      {
        "title": "Tesla Run (from the video game 'Fracture')",
        "url": "/music/christilton_fracture_03_tesla-run.mp3" 
      },
      {
        "title": "The Dreadnaught Attack (from the video game 'Fracture')",
        "url": "/music/christilton_fracture_04_the-dreadnaught-attack.mp3" 
      },
      {
        "title": "The Dreadnaught Falls (from the video game 'Fracture')",
        "url": "/music/christilton_fracture_05_the-dreadnaught-falls.mp3"
      }
    ]
  },
  { 
    id: 14, 
    ctalink: "/projects", 
    project: "Mercenaries 2: World in Flames", 
    company: "Electronic Arts/Pandemic", 
    src: "/images/cover-art/mercenaries-2.jpg", 
    hero: "bg-[url('/images/hero-mercenaries-2.jpg')]",
    title: "Mercenaries 2: World in Flames (Video Game) 2008", 
    color: "bg-[#849CA8]", 
    subtitle: "", 
    cta: "Listen on Spotify", 
    ctabrand: "bg-[#1DB954] hover:bg-[#1DB954]",
    ctabrandhover: "",
    ctaicon: "icon-spotify.png", 
    year: "2008", 
    desc: "Mercenaries 2: World in Flames is an action-adventure video game developed by Pandemic Studios. Set in an open-world environment, players take on the role of mercenaries tasked with completing various missions and objectives in a war-torn Venezuela, using a wide range of vehicles, weapons, and tactics to achieve their goals.", 
    songsdata: [
      {
        "title": "Main Theme (from the video game 'Mercenaries 2: World in Flames') [Excerpt]",
        "url": "/music/christilton_mercs2_01_mercs2-main-theme_excerpt.mp3" 
      },
      {
        "title": "Streets of Venezuela (from the video game 'Mercenaries 2: World in Flames') [Excerpt]",
        "url": "/music/christilton_mercs2_02_streets-of-venezuela_excerpt.mp3" 
      },
      {
        "title": "Jungle Brawl (from the video game 'Mercenaries 2: World in Flames') [Excerpt]",
        "url": "/music/christilton_mercs2_03_jungle-brawl_excerpt.mp3" 
      },
      {
        "title": "Battle at Caracas (from the video game 'Mercenaries 2: World in Flames') [Excerpt]",
        "url": "/music/christilton_mercs2_04_battle-at-caracas_excerpt.mp3" 
      },
      {
        "title": "End Credits (from the video game 'Mercenaries 2: World in Flames') [Excerpt]",
        "url": "/music/christilton_mercs2_05_end-credits_excerpt.mp3"
      }
    ]
  },

  {
    id: 15,
    ctalink: "/projects",
    project: "Jumper",
    company: "Brash Entertainment/Redtribe",
    src: "/images/cover-art/jumper.jpg",
    hero: "bg-[url(/images/hero-jumper.jpg)]",
    title: "Jumper (Video Game) 2008",
    color: "bg-[#3D7B89]",
    subtitle: "",
    cta: "Download Soundtrack",
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "icon-download.png",
    year: "2008",
    desc: "Jumper: Griffin's Story is an action-adventure video game based on the movie Jumper. Developed by Red Tribe and published by Brash Entertainment, the game follows the story of Griffin, a new character introduced exclusively for the game. Players take on the role of Griffin, a teleporting warrior, as he embarks on a quest for revenge against an ancient secret society, utilizing his unique abilities to battle enemies across various locations and unravel the mysteries of the Jumper universe.",
    songsdata: [
      { title: "Griffin's Story (from the video game 'Jumper: Griffin's Story')", url: "/music/christilton_jumper_01_griffins-story.mp3" },
      { title: "Path to Roland (from the video game 'Jumper: Griffin's Story')", url: "/music/christilton_jumper_02_path-to-roland.mp3" },
      { title: "Network Breach (from the video game 'Jumper: Griffin's Story')", url: "/music/christilton_jumper_03_network-breach.mp3" },
      { title: "Tokyo Dockside (from the video game 'Jumper: Griffin's Story')", url: "/music/christilton_jumper_04_tokyo-dockside.mp3" },
      { title: "Final Confrontation (from the video game 'Jumper: Griffin's Story')", url: "/music/christilton_jumper_05_final-confrontation.mp3" }
    ]
  },
  
  {
    id: 16,
    ctalink: "/projects",
    project: "La Lune",
    company: "Directed by Kelly Kyung-Hee Shon",
    src: "/images/cover-art/la-lune.jpg",
    hero: "bg-[url(/images/hero-la-lune.jpg)]",
    title: "La Lune (Short) 2008",
    color: "bg-[#F2A04F]",
    subtitle: "",
    cta: "Download Soundtrack",
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "icon-download.png",
    year: "2008",
    desc: "In this magical fairy tale, two minature lovers seek the sun from atop a crumbling hourgalss.",
    songsdata: [
      { title: "Opening (from the short animation 'La Lune')", url: "/music/christilton_lalune_01_opening.mp3" },
      { title: "Shoot for the Lune (from the short animation 'La Lune')", url: "/music/christilton_lalune_02_shoot-for-the-lune.mp3" },
      { title: "The Hourglass (from the short animation 'La Lune')", url: "/music/christilton_lalune_03_the-hourglass.mp3" },
      { title: "End Titles (from the short animation 'La Lune')", url: "/music/christilton_lalune_04_end-titles.mp3" },
      { title: "La Lune Full Score (from the short animation 'La Lune')", url: "/music/christilton_lalune_05_la-lune-full-score.mp3" }
    ]
  },
  {
    id: 17,
    ctalink: "/projects",
    project: "The Sunday Man",
    company: "Directed by Danielle Shamash",
    src: "/images/cover-art/the-sunday-man.jpg",
    hero: "bg-[url(/images/hero-the-sunday-man.jpg)]",
    title: "The Sunday Man (Short) 2007",
    color: "bg-[#A86E52]",
    subtitle: "",
    cta: "Download Soundtrack",
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "icon-download.png",
    year: "2007",
    desc: "The misadventures of a remarkably peculiar man, who has some very unusual ideas about who he is.",
    songsdata: [
      { title: "Opening Titles (from the short film 'The Sunday Man')", url: "/music/christilton_sundayman_01_opening-titles.mp3" },
      { title: "My Mom's a Hugger (from the short film 'The Sunday Man')", url: "/music/christilton_sundayman_02_my-moms-a-hugger.mp3" },
      { title: "Bible Study (from the short film 'The Sunday Man')", url: "/music/christilton_sundayman_03_bible-study.mp3" },
      { title: "Miracle Man (from the short film 'The Sunday Man')", url: "/music/christilton_sundayman_04_miracle-man.mp3" },
      { title: "The Sunday Man (from the short film 'The Sunday Man')", url: "/music/christilton_sundayman_05_the-sunday-man.mp3" }
    ]
  },
  {
    id: 18,
    ctalink: "/projects",
    project: "Black",
    company: "Electronic Arts/Criterion Games",
    src: "/images/cover-art/black.jpg",
    hero: "bg-[url('/images/hero-black.jpg')]",
    title: "Black (Video Game) 2006",
    color: "bg-[#4B4A4A]",
    subtitle: "Main Theme co-composed with Michael Giacchino",
    cta: "Listen on Spotify",
    ctabrand: "bg-[#1DB954] hover:bg-[#1DB954]",
    ctabrandhover: "",
    ctaicon: "icon-spotify.png",
    year: "2006",
    desc: "Black is a first-person shooter video game developed by Criterion Games and published by Electronic Arts (EA). Known for its intense and explosive action, the game immerses players in high-octane combat scenarios, where they unleash a vast arsenal of realistic and powerful weaponry to take on enemies and complete challenging missions in a stunningly detailed environment.",
    songsdata: [
      {
        "title": "Main Theme (from the video game 'Black') [Excerpt]",
        "url": "/music/christilton_black_01_main-theme_excerpt.mp3"
      },
      {
        "title": "Tunnel Trouble (from the video game 'Black') [Excerpt]",
        "url": "/music/christilton_black_02_tunnel-trouble_excerpt.mp3"
      },
      {
        "title": "Madhouse Mayhem (from the video game 'Black') [Excerpt]",
        "url": "/music/christilton_black_03_madhouse-mayhem_excerpt.mp3"
      },
      {
        "title": "Gulag Gauntlet (from the video game 'Black') [Excerpt]",
        "url": "/music/christilton_black_04_gulag-gauntlet_excerpt.mp3"
      },
      {
        "title": "Bunker Buster (from the video game 'Black') [Excerpt]",
        "url": "/music/christilton_black_05_bunker-buster_excerpt.mp3"
      }
    ]
  },
  {
    id: 19,
    ctalink: "/projects",
    project: "Mercenaries",
    company: "LucasArts/Pandemic",
    src: "/images/cover-art/mercenaries.jpg",
    hero: "bg-[url('/images/hero-mercenaries.jpg')]",
    title: "Mercenaries (Video Game) 2005",
    color: "bg-[#736953]",
    subtitle: "Co-composed with Michael Giacchino",
    cta: "",
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "",
    year: "2005",
    desc: "Mercenaries is an action-adventure video game developed by Pandemic Studios. Set in a fictional war-torn North Korea, players assume the roles of skilled mercenaries, taking on dangerous missions for different factions and governments, with a vast array of weaponry, vehicles, and destructive capabilities at their disposal.",
    songsdata: [
      {
        "title": "Gas Tank on the Roof (from the video game 'Mercenaries')",
        "url": "/music/christilton_mercs_01_gas-tank-on-the-roof.mp3"
      },
      {
        "title": "For the Motherland (from the video game 'Mercenaries')",
        "url": "/music/christilton_mercs_02_for-the-motherland.mp3"
      },
      {
        "title": "Hidden Valley Bunker (from the video game 'Mercenaries')",
        "url": "/music/christilton_mercs_03_hidden-valley-bunker.mp3"
      },
      {
        "title": "Countdown (from the video game 'Mercenaries')",
        "url": "/music/christilton_mercs_04_countdown.mp3"
      },
      {
        "title": "Ending (from the video game 'Mercenaries')",
        "url": "/music/christilton_mercs_05_ending.mp3"
      }
    ]
  },
  {
    id: 20,
    ctalink: "/projects",
    project: "Muppets Party Cruise",
    company: "Mass Media/TDK",
    src: "/images/cover-art/muppets-party-cruise.jpg",
    hero: "bg-[url('/images/hero-muppets.jpg')]",
    title: "Muppets Party Cruise (Video Game) 2003",
    color: "bg-[#BF9D64]",
    subtitle: "",
    cta: "",
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "",
    year: "2003",
    desc: "Published by TDK Mediactive, Muppets Party Cruise offers a fun-filled multiplayer experience where players can participate in a variety of wacky mini-games and challenges set in the quirky Muppets universe, with humor and antics abound.",
    songsdata: [
      {
        "title": "The Cruise (from the video game 'Muppets: Party Cruise')",
        "url": "/music/christilton_muppets_01_the-cruise.mp3"
      },
      {
        "title": "Stinky Cheese (from the video game 'Muppets: Party Cruise')",
        "url": "/music/christilton_muppets_02_stink-cheese.mp3"
      },
      {
        "title": "Scuba Chicken (from the video game 'Muppets: Party Cruise')",
        "url": "/music/christilton_muppets_03_scuba-chicken.mp3"
      },
      {
        "title": "Wocka Wocka (from the video game 'Muppets: Party Cruise')",
        "url": "/music/christilton_muppets_04_wocka-wocka.mp3"
      },
      {
        "title": "Wet and Green (from the video game 'Muppets: Party Cruise')",
        "url": "/music/christilton_muppets_05_wet-and-green.mp3"
      }
    ]
  },
  {
    id: 21,
    ctalink: "",
    project: "Personal",
    company: "by Chris Tilton",
    src: "/images/cover-art/chris-conducting.jpg",
    hero: "bg-[url('/images/hero-personal.jpg')]",
    title: "Personal",
    color: "bg-[#CDBE70]",
    subtitle: "",
    cta: "",
    ctabrand: "",
    ctabrandhover: "",
    ctaicon: "",
    year: "",
    desc: "",
    songsdata: [
      {
        "title": "The Encounter on Ord Mantell (a Chris Tilton original)",
        "url": "/music/christilton_ctoriginal_01_the-encounter-on-ord-mantell.mp3"
      },
      {
        "title": "Oscillating Rift (a Chris Tilton original)",
        "url": "/music/christilton_ctoriginal_02_oscillating-rift.mp3"
      },
      {
        "title": "The Road Home Again (a Chris Tilton original)",
        "url": "/music/christilton_ctoriginal_03_the-road-home-again.mp3"
      },
      {
        "title": "Fox Hunt (a Chris Tilton original)",
        "url": "/music/christilton_ctoriginal_04_fox-unit.mp3"
      },
      {
        "title": "The Promise (a Chris Tilton original)",
        "url": "/music/christilton_ctoriginal_05_the-promise.mp3"
      }
    ]
  },
  
      
              
  { 
    homepage: true,
    hyperlink: true,
    homepageonly: true,
    hyperlinkurl: "/music",
    project: "More projects", 
    src: "/images/ig-10.jpg", 
    songsdata: [
      {
        "title": "Main Theme (from the video game 'Black')",
        "url": "/music/christilton_from_01_from-main-theme.mp3" 
      },
      {
        "title": "Driving in Circles (from the TV series 'From')",
        "url": "/music/christilton_from_02_driving-in-circles.mp3" 
      },
      {
        "title": "Stone in the Cellar (from the TV series 'From')",
        "url": "/music/christilton_from_03_stone-in-the-cellar.mp3" 
      },
      {
        "title": "Pray with Me (from the TV series 'From')",
        "url": "/music/christilton_from_04_pray-with-me.mp3" 
      },
      {
        "title": "Messages in the Bottles (from the TV series 'From')",
        "url": "/music/christilton_from_05_messages-in-the-bottles.mp3"
      }
    ],
  },
  
]
